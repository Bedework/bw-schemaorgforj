...
import org.bedework.schemaorg.impl.values.SOThingImpl;
 ...
 */
public class SOThingFactory extends JsonValueFactoryImpl {
  @Override
  public JsonValue newValue(final JsonFactory factory,
                            final String typeName,
                            final JsonNode nd) {
    if (nd != null) {
      return new SOThingImpl(factory, typeName,
                             ensureType(typeName,
                                        (ObjectNode)nd));
    }

    return new SOThingImpl(factory, typeName, newObject(typeName));
  }
}
